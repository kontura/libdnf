...
void CmdUpdateinfo::set_argument_parser(Context & ctx) {
    ...
    updates->set_short_description("show advisories about newer versions of installed packages for which a newer version is available");
    ...
    specs->set_short_description("List of specs to use when searching for updateinfo");
    ...
    auto updateinfo = ctx.arg_parser.add_new_command("updateinfo");
    updateinfo->set_short_description("display information about update advisories");
    updateinfo->set_description("");
    updateinfo->set_named_args_help_header("Optional arguments:");
    updateinfo->set_positional_args_help_header("Positional arguments:");
    updateinfo->set_parse_hook_func([this, &ctx](
    ...
    updateinfo->register_named_arg(all);
    updateinfo->register_named_arg(available);
    updateinfo->register_named_arg(installed);
    updateinfo->register_named_arg(updates);

    updateinfo->register_named_arg(summary);
    updateinfo->register_named_arg(list);
    updateinfo->register_named_arg(info);

    updateinfo->register_named_arg(with_cve);
    updateinfo->register_named_arg(with_bz);

    updateinfo->register_positional_arg(specs);


    ctx.arg_parser.get_root_command()->register_command(updateinfo);
}

void CmdUpdateinfo::run(Context & ctx) {
    ...
    //TODO(amatej): Do I need all the flags here?
    ...
    if (patterns_to_show.size() > 0) {
        solv_query.ifilter_name(QueryCmp::IGLOB, patterns_to_show);
    }

    auto & advisory_sack = ctx.base.get_rpm_advisory_sack();
    ...
    auto advisory_query = advisory_sack.new_query();
    ...
    std::vector<libdnf::AdvisoryPackage> result_pkgs;

    if (availability_option->get_value() == "installed") {
        auto installed_solv_query = solv_query.ifilter_installed();
        result_pkgs = advisory_query.get_advisory_packages(QueryCmp::LTE, installed_solv_query);
    } else if (availability_option->get_value() == "available") {
        //TODO(amatej): filter for latest and add kernel..
        auto installed_solv_query = solv_query.ifilter_installed();
        result_pkgs = advisory_query.get_advisory_packages(QueryCmp::GT, installed_solv_query);
    } else if (availability_option->get_value() == "all") {
        auto installed_solv_query = solv_query.ifilter_installed();
        result_pkgs = advisory_query.get_advisory_packages(QueryCmp::LT | QueryCmp::EQ | QueryCmp::GT, installed_solv_query);
    } else if (availability_option->get_value() == "updates") {
        //auto upgradable_solv_query = solv_query.ifilter_upgradable().ifilter_nevra(QueryCmp::GT, keys);
        //result_pkgs = advisory_query.get_advisory_packages(QueryCmp::LT | QueryCmp::EQ | QueryCmp::GT, upgradable_solv_query);
    }
    ...
    for (auto pkg : result_pkgs) {
        ...
        std::cout << pkg.get_name() << "-" << pkg.get_evr() << "." << pkg.get_arch() << std::endl;
        //} else {
    ...
    //for filterAdvisory use case
    //std::vector<std::string> nevras = advisory_query.get_advisory_pkgs_nevras();
    //solv_query = solv_query.ifilter_upgrade().ifilter_nevra(QueryCmp::LTE, nevras);
